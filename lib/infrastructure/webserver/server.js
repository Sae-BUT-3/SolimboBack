...
const createServer = async () => {
  ...
  console.log(__dirname.split(Path.sep).slice(0,-3).join(Path.sep)+'/upload')
  const server = Hapi.server({
    port: process.env.PORT || 3000,
    routes:{
      files: {
        relativeTo: process.env.UPLOAD_DIR || Path.join(__dirname.split("\\").slice(0,-3).join("/")+'/upload')
      }
    }
  });
  ...
  await server.register([
    ...
    require('../../interfaces/routes/notification')
  ]);

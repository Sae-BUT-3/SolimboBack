...
module.exports = class extends userRepository {
  ...
  async getByIdent(ident) {
    const seqUser = await this.model.findOne({
      where: {
        [Op.or]: [{ pseudo: ident }, { email: ident }],
      },
    });
    return this.createUser(seqUser);
  }
    ...
    const seqUser = await this.model.findOne({
      attributes: {
        include: [
          [
            sequelize.literal(
              "(SELECT COUNT(*) FROM amis WHERE amis.id_utilisateur = utilisateur.id_utilisateur AND en_attente=false)"
            ),
            "following_count",
          ],
          [
            sequelize.literal(
              "(SELECT COUNT(*) FROM amis WHERE amis.amiIdUtilisateur = utilisateur.id_utilisateur AND en_attente=false)"
            ),
            "follower_count",
          ],
          [
            sequelize.literal(
              "(SELECT COUNT(*) FROM review WHERE review.id_utilisateur = utilisateur.id_utilisateur)"
            ),
            "review_count",
          ],
        ],

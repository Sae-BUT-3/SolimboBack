...
const getPreWhere = (fetchPrivate, id, friendsOnly) => {
  ...
  return {
    whereClause,
    replacements: { id: id }, //.sera remplace par la fct qui recupere via sequalize
  }
};
module.exports = class extends ReviewRepository {
  ...
  async getReviews(page, pageSize, orderByLike, fetchPrivate, friendsOnly, id) {
    ...
    let {whereClause, replacements} = getPreWhere(fetchPrivate, id, friendsOnly);
    const reviews = await this.review.findAll({
      ...
      order,
      where: whereClause,
      replacements: replacements, // remplace les parametres
    });
  ...
  async geTimelineReviewCount(fetchPrivate, id) {
    let {whereClause : where, replacements}= getPreWhere(fetchPrivate, id, false);
    let whereClause = {
      [Op.or]: [
        where,
        {

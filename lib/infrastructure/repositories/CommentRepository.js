...
module.exports = class extends CommentRepository {
  ...
  async getCommentsComments(id_reponse,id_utilisateur,fetchPrivate,page,pageSize,orderByLike) {
    let whereClause = {
      id_reponse,
      deleted: false
    }
    ...
    const comments = await this.comment.findAll({
      ...
      attributes: {
        include: [
          ...
          [
            sequelize.literal('(SELECT COUNT(*) FROM commentaire AS comm2 WHERE comm2.id_reponse = commentaire.id_com)'),
            'countComment'
          ],
        ],
      },
  ...
  async getById(id_com,deleted = false) {
    ...
    return comment
    ? this.createComment(comment.dataValues)
    : null
  }

...
const User = require('../../domain/model/UserPublic');
...
module.exports = class  extends FriendRepositoryAbstract { 
  ...
  async getFollowInfo(id_utilisateur, amiIdUtilisateur) {
    ...
    const relation1 = await this.model.findOne({ //if user follows the other
      id_utilisateur: id_utilisateur,
      amiIdUtilisateur: amiIdUtilisateur,
    });
    const relation2 = await this.model.findOne({ //if the other follows the user
      id_utilisateur: amiIdUtilisateur,

...
const userSignUp = Joi.object().keys({
    email: Joi.string().email().min(10).max(40).required().error(validationErrror("email","l'email est invalide")),
    ...
    alias: Joi.string().min(3).max(15).required().error(validationErrror("alias","l'alias doit être compris entre 3 et 15 caractère")),
    ...
    spotifyToken: Joi.string().max(1000),

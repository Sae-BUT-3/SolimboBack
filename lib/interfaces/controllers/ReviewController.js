...
module.exports = {
    ...
    async getOeuvreReviews(request,handler){
        try{
            ...
            const {id} = request.params
            const {page, pageSize,orderByLike} = request.query
            return handler.response(await getOeuvreReviews(id, token,page,pageSize,orderByLike, serviceLocator)).code(200)
        }catch(error){
            return handleError(error)
        }
    },
    async getArtistReviews(request,handler){
        try{
            ...
            const {id} = request.params
            const {page, pageSize,orderByLike} = request.query
            return handler.response(await getArtistReviews(id, token,page,pageSize,orderByLike, serviceLocator)).code(200)
        }catch(error){
            return handleError(error)
        }
    },
    ...
    async getOeuvreReviews(request,handler){
        try{
            ...
            const {page, pageSize,orderByLike} = request.query
            const response =  {
                data: await getOeuvreReviews(id, token,page,pageSize,orderByLike, serviceLocator),

...
module.exports = {
    ...
    async confirmUser(request, handler) {
        try{
            ...
            const user = await CompleteAccount(pseudo, alias, bio,photo,confirmToken, serviceLocator);
    ...
    async authWithSpotify(request, handler){
        const serviceLocator = request.server.app.serviceLocator;
        const {spotify_code} = request.payload
        try{
            const returnValue = await AuthWithSpotify(spotify_code,serviceLocator);
            if(returnValue.idUtilisateur){
                returnValue(returnValue.idUtilisateur,false,serviceLocator)

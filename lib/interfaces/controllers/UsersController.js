...
module.exports = {
    ...
    async oeuvreFav(request, handler){
        ...
        try{
           // await oeuvreFav(idOeuvre,serviceLocator);
            const returnValue = await oeuvreFav(token,idOeuvre,type,serviceLocator)
    ...
    async getPage(request, handler){
        ...
        return handler.response(await getPage(id,page,pageSize,orderByLike,token,serviceLocator)).code(200)
        }
        catch (e){
            return handleError(e)
        }
    ...
    async modify(request, handler){
        try{
            console.log("fezfzefz")
            console.log(request.info.protocol)
            console.log(request.info.hostname)
            console.log
            const baseURL = `${request.info.protocol}//${request.info.hostname}:${request.connection.info.port}`
            console.log(baseURL)
            const serviceLocator = request.server.app.serviceLocator;
            const authorizationHeader = request.headers.authorization;
            const [, token] = authorizationHeader.split(' ');
            const {photo} = request.payload
        return handler.response(await modifyProfile(photo,token,serviceLocator)).code(200)
        }
        catch (e){
            return handleError(e)
        }

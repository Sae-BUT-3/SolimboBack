...
module.exports = async (
  token,
  amiIdUtilisateur,
  { accessTokenManager, friendRepository, userRepository, mailRepository,notificationRepository }
) => {
  ...
  const notication = deserializeNotification("follow",id,amiIdUtilisateur)
  const followInfo = await friendRepository.getFollowInfo(id, amiIdUtilisateur);
  if (followInfo.doesFollows) {
    await notificationRepository.destroy(notication)
  ...
  if (ami.is_private) {
    ...
    console.log("ttttttttttttgfeezefz")
    await notificationRepository.persist(notication)
  }
  return friend;
};

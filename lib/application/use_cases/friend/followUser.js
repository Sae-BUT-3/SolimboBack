...
module.exports = async (token, amiIdUtilisateur, {accessTokenManager, friendRepository, userRepository, mailRepository}) => {
    ...
    if (!friend) {
        throwStatusCode(403, 'Vous avez déjà  ajouté cet utilisateur.')
    }
    if (ami.is_private) {
        const mailOptions = {

...
const serializeLike = (notification, notications, key) => {
  if (notications[key]) {
    ...
    if(notications[key].createdAt.getTime() < notification.createdAt.getTime()){
      notications[key].createdAt = notification.createdAt
    }
    return
  }
...
module.exports = async (
  ...
  .sort((a, b) => {
    if(isNaN(a.createdAt)) return 1
    if(isNaN(b.createdAt)) return -1

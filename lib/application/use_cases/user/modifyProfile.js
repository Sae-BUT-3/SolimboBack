...
module.exports = async (
  ...
  if (file) {
    ...
    if (previewPath) {
      await documentRepository.deleteFile(`${process.env.UPLOAD_DIR || "./upload"}/${previewPath}`);
    }
    const path = await documentRepository.uploadFile(process.env.UPLOAD_DIR || "./upload", file);

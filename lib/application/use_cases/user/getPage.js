...
module.exports = async (
  ...
  console.log("dfzadfzdzadazdazdza");
  ...
  const idLikeOeuvres = await likeOeuvreRepository.getOeuvreLikes(
    id_utilisateur
  );
  ...
  const usersfollowed = await friendRepository.getListFriendsById(
    id_utilisateur
  );
  ...
  const artistfollowed = await followRepository.getArtistFollowed(
    id_utilisateur
  );
  ...
  const oeuvres = await Promise.all(
    idOeuvres.map(async (oeuvre) => {
      ...
      SpotifyOeuvre.reviewCount = await reviewRepository.getCountReviewByOeuvre(
        oeuvre.id_oeuvre
      );
  ...
  const reviews = reviewsRaw.map((element) => {
  ...
  return {
    ...
    followers: followers.map((item) => new UserPublic(item)),
    forbidden: selected_user.is_private && !doesFollows && !isCurrent,
    isCurrent: isCurrent,
    relation: relation,
    oeuvres: oeuvres || [],
    likeOeuvres: await Promise.all(likeOeuvres) || [],

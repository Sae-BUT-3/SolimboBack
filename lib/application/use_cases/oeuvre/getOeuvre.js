...
const trackSerializer = require("../../../interfaces/serializers/TrackSerializer.js");
...
module.exports = async (
  ...
  const artists = await Promise.all(
    artistIds.map(async (id) => {
      ...
      const tracks = await spotifyRepository.getSpotifyTopTrackArtist(artist.id);
      tracks?.tracks.map(item => item.type !== 'track' ?  topTracks.push(trackSerializer(item)) : null);
  ...
  oeuvre.topTracks = shuffle(topTracks);

...
module.exports = async (artistId, userToken,page,pageSize, orderByLike, {reviewRepository, spotifyRepository,accessTokenManager}) => {
    let id = null
    if (userToken) {
    ...
    const albums = await spotifyRepository.getSpotifyArtistSongs(artistId,'album,single')
    const album_ids = albums.items.map((album)=>album.id)
    const reviews = await reviewRepository.getOeuvreReviews(page,pageSize, orderByLike,false,album_ids,id)

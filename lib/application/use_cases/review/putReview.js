...
module.exports = async (
  ...
  if (!(await oeuvreRepository.is_oeuvre_relation(idOeuvre))) {
    const artist_ids = rawOeuvre.artists.map((item) => item.id);
    await oeuvreRepository.persist(idOeuvre, artist_ids);
  }
  const ReviewRaw = {
    id_oeuvre: idOeuvre,
    id_utilisateur,
